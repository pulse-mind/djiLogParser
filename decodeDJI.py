...
def write2csv():
    root = os.getcwd()
    dir = os.path.join(root, "data/output/csv")
    if not os.path.exists(dir):
        os.makedirs(dir)

    for item in DJIFrame:
        if len(DJIFrame[item]) > 0:
            with open(os.path.join(dir,item+".csv"),'w', newline='') as csvfile:
...
def write2jpg():
    if len(DJIFrame['JPG']) == 2:
        root = os.getcwd()
        dir = os.path.join(root, "data/output/images")
        if not os.path.exists(dir):
            os.makedirs(dir)
        image_index = 0
        for image_name in DJIFrame['JPG'][0]:
            if image_name.startswith("Pic_"):
                image_content = DJIFrame['JPG'][1][image_index]
                with open(os.path.join(dir,image_name+".jpg"), 'wb') as f:
...
def JPG(payload):
    ...
    for x in payload:
        ...
        if ((lastByte<<8)|x) == 0xFFD9:
            #resultPic[picIndex].append(255) // Do not add this one because it was added during last loop
            resultPic[picIndex].append(217)
            picIndex = picIndex + 1
        #if x == 0xff:
        #    return 0
        if len(resultPic) > 0 and picIndex < len(resultPic):
...
def parserBody(body,version,detail,recordEnd):
    ...
    while i < len(recordArea):
        ...
        #JPG format
        if record_type == 57:
            print("Dealing with frame", frameNumber, " with type (", record_type, "): ", DJIFrameList[record_type])
            JPG(recordArea[i+2:])
            break
        else:
...
# @(default)	this	this	make up 4 bytes
# =	Native	standard 	none	according to the original bytes
# <	Little endian standard	none,according to the original number of bytes
# >	Big endian standard none, according to the original number of bytes
# !	network (big endian) standard none, according to the original number of bytes


# Format		C Language type		Python Type of		Standard size
# x			pad byte		no value
...
# i			int				integer				4
# I (uppercase i)	unsigned int	integer				4
